...
- `adr/` — Architecture Decision Records (expensive-to-reverse choices)
  ...
  - `0008-modular-multi-provider-screening.md`
...
Copy `D:\projects\guide\templates\adr.md` to `docs/adr/NNNN-title.md`, number from `0008`, fill `Date`, `Status`, `Context`, `Options`, `Decision`, `Consequences`. ADR is not Rust-only — use for protocol, data-model, or infra choices that are costly to undo. Small decisions go in the Issue.
