...
- Config: `src/config.py` only (no `os.environ` in feature code except `bot.py:52` for `PORT`)
- Data: `data/stock_screener.db` (gitignored, `data/*.db` + `data/oracle_cloud`), `logs/stock_screener.log` (gitignored)
- Secrets: `.env` (gitignored) — never commit `TELEGRAM_BOT_TOKEN` or `GEMINI_API_KEY`; template is `.env.example`
